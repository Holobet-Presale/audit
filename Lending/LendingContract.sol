...
// File: contracts/Lending/LendingContract.sol
...
contract HBTLending is Ownable {
    ...
    function setInterestFeeDays(uint256 newDuration) public onlyOwner {
    ...
    function getUserInterest(address user) public view returns (uint256) {
        ...
        Loan storage loan = loans[user];
        bool isInterestFee = (loan.loanTime + loan.interestFreeDays) >
            block.timestamp;
        if (loan.amount > 0 && !isInterestFee) {
    ...
    function repayLoan() external {
        ...
        uint256 interestAmount = getUserInterest(msg.sender);
        if (interestAmount > stakedAmount) {
            interestAmount = stakedAmount;
        }
        if (interestAmount > 0) {
