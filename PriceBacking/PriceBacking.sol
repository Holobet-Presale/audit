...
// File: contracts/PriceBacking/PriceBacking.sol
...
contract PriceBacking is Ownable {
    ...
    function HBTToUSDCRate() public view returns (uint256) {
        ...
        uint256 hbtInUSDCDecimals = HBTCirculation / 1e12;
        return ((USDCBalance) * 1e6) / hbtInUSDCDecimals;
    }

    function HBTToUSDC(uint256 hbtAmount) public view returns (uint256) {
        uint256 rate = HBTToUSDCRate();
        uint256 hbtInUSDCDecimals = hbtAmount / 1e12;
        return (rate * hbtInUSDCDecimals) / 1e6;
    }
